defmodule Protohackers.SpeedCamera.MessageTest do
  ...
  describe "encode/1 + decode/1 for all messages" do
    ...
    test "TicketDispatcher" do
      message = %Message.TicketDispatcher{roads: [582]}
      assert {:ok, ^message, ""} = Message.decode(Message.encode(message))
    end

    test "Camera" do
      message = %Message.Camera{road: 582, mile: 4452, limit: 100}
      assert {:ok, ^message, ""} = Message.decode(Message.encode(message))
    end
